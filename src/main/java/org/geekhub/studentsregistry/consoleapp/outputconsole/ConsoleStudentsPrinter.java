...
public class ConsoleStudentsPrinter {
    ...
    public void printStudentsTable(GradeType gradeType, List<HashMap<String, String>> studentsForPrinting) {
        if(isParametersValid(gradeType, studentsForPrinting)) {
            System.out.printf("\nWith %s Grade %s\n",
    ...
    private boolean isParametersValid(GradeType gradeType, List<HashMap<String, String>> studentsForPrinting) {
        if (gradeType == null) throw new IllegalArgumentException(
                "Grade type for printing is null");
        else if (studentsForPrinting == null) throw new IllegalArgumentException(
                "Students list for printing is null");
        else return true;
    }
