...
public class StudentsRegistryCreator {
    ...
    private Object getObjectFromInterface(Class<?> interfaceClass, List<Class<?>> classesForNewInstance)
        ...
        Class<?> classNeededScanner = implementClasses.stream()
                .filter(this::isThisClassNeedsScanner).findAny().orElse(null);
        return null == classNeededScanner
                ? getNewInstanceWithoutParameters(implementClasses.stream().filter(cl -> !cl.isInterface()).findFirst().orElseThrow())
                : getNewInstanceWithParameter(classNeededScanner, scanner);
    }
